...
const DAY = 86400;
$configureUrl = 'index.php?controller=AdminModules&configure=priceandorder';
...
$unprefixed = array_filter($keys, function ($k) { return strpos($k, 'PRICEANDORDER_') !== 0; });
ok($unprefixed === [], 'all three carry the PRICEANDORDER_ prefix (' . implode(', ', $keys) . ')');
ok(MegVentureReviewNudge::REVIEW_URL === 'https://megventure.com/{lang}/testimonials/write?id_product=10',
   'REVIEW_URL targets product id 10 on megventure.com');

echo "\n2) Fresh timestamp: hidden on day 0\n";
Configuration::$store = [];
ok(MegVentureReviewNudge::onInstall(), 'onInstall() succeeds');
$installedAt = (int) Configuration::get('PRICEANDORDER_REVIEW_INSTALLED_AT');
...
ok((int) Configuration::get('PRICEANDORDER_REVIEW_DISPLAYS') === 0, 'a hidden page view does not count as a display');

echo "\n3) Quiet period: day 20 hidden, day 21 shown\n";
Configuration::$store['PRICEANDORDER_REVIEW_INSTALLED_AT'] = (string) (time() - 20 * DAY);
ok(!MegVentureReviewNudge::shouldDisplay(), 'day 20: hidden');
Configuration::$store['PRICEANDORDER_REVIEW_INSTALLED_AT'] = (string) (time() - 21 * DAY);
ok(MegVentureReviewNudge::shouldDisplay(), 'day 21: shown');
Configuration::$store['PRICEANDORDER_REVIEW_INSTALLED_AT'] = (string) (time() - 100 * DAY);
...
ok(strpos($html, 'priceandorder_review_go=1') !== false, 'render() contains the review link');
ok(strpos($html, 'priceandorder_review_dismiss=1') !== false, 'render() contains the dismiss link');
ok(strpos($html, 'megventure.com') === false, 'no direct external URL in the HTML (click routes through the configure page)');
ok((int) Configuration::get('PRICEANDORDER_REVIEW_DISPLAYS') === 1, 'the view was counted');

echo "\n5) Three unanswered displays, hidden on the fourth\n";
Configuration::$store['PRICEANDORDER_REVIEW_DISPLAYS'] = '0';
...
ok((int) Configuration::get('PRICEANDORDER_REVIEW_DISPLAYS') === 3, 'counter stopped at 3');

echo "\n6) A form-POST re-render is shown but not counted\n";
Configuration::$store['PRICEANDORDER_REVIEW_DISPLAYS'] = '1';
$_POST['submitModuleSettings'] = '1';
ok(MegVentureReviewNudge::render($fake, $configureUrl) !== '', 'still shown while saving settings');
ok((int) Configuration::get('PRICEANDORDER_REVIEW_DISPLAYS') === 1, 'but the save re-render did not burn a display');
$_POST = [];

echo "\n7) Dismissed: hidden forever\n";
Configuration::$store['PRICEANDORDER_REVIEW_DISPLAYS'] = '0';
Tools::$values = ['priceandorder_review_dismiss' => '1'];
...
ok((int) Configuration::get('PRICEANDORDER_REVIEW_DISMISSED') === 1, 'dismissed flag written');
ok(!MegVentureReviewNudge::shouldDisplay(), 'hidden right after dismissing');
Configuration::$store['PRICEANDORDER_REVIEW_INSTALLED_AT'] = (string) (time() - 365 * DAY);
ok(!MegVentureReviewNudge::shouldDisplay(), 'hidden a year later too');

echo "\n8) Review link clicked: recorded, redirected, hidden forever\n";
Configuration::$store = ['PRICEANDORDER_REVIEW_INSTALLED_AT' => (string) (time() - 30 * DAY)];
Tools::$redirectedTo = null;
Tools::$values = ['priceandorder_review_go' => '1'];
MegVentureReviewNudge::handleRequest($fake);
Tools::$values = [];
ok(Tools::$redirectedTo === 'https://megventure.com/tr/testimonials/write?id_product=10',
...
ok(MegVentureReviewNudge::reviewUrl('xx') === 'https://megventure.com/en/testimonials/write?id_product=10',
...
Configuration::$store = ['PRICEANDORDER_OTHER' => '1']; // a 2.0.x shop: no review keys at all
ok(upgrade_module_2_1_0(null) === true, 'upgrade script succeeds');
$stamp = (int) Configuration::get('PRICEANDORDER_REVIEW_INSTALLED_AT');
...
Configuration::$store['PRICEANDORDER_REVIEW_INSTALLED_AT'] = '12345';
upgrade_module_2_1_0(null);
ok(Configuration::get('PRICEANDORDER_REVIEW_INSTALLED_AT') === '12345', 'an existing timestamp is never overwritten');

echo "\n10) onUninstall removes the three keys and touches nothing else\n";
Configuration::$store = [
    'PRICEANDORDER_REVIEW_INSTALLED_AT' => '12345',
    'PRICEANDORDER_REVIEW_DISMISSED' => '1',
    'PRICEANDORDER_REVIEW_DISPLAYS' => '2',
    'PRICEANDORDER_OTHER' => 'tok',
    'theme' => 'another-modules-value',
    'MEGTESTIMONIAL_WHO' => 'another-modules-value',
];
MegVentureReviewNudge::onUninstall();
$reviewLeft = array_filter(array_keys(Configuration::$store), function ($k) { return strpos($k, 'PRICEANDORDER_REVIEW_') === 0; });
ok($reviewLeft === [], 'onUninstall() removed all three review keys');
ok(Configuration::get('PRICEANDORDER_OTHER') === 'tok', 'the module\'s other keys were left for the module');
...
$src = (string) file_get_contents(dirname(__DIR__) . '/priceandorder.php');
